// EWC2.5 packet decoder — server-side copy of ewater-app/src/lib/ewc25.ts
// Adapted for Node.js: atob() replaced with Buffer.from(..., 'base64').
...
export function decodeEwc25(hexPayload: string, lcf?: number | null): Ewc25Result {
  ...
  if (event === 0x18) {
    return { ...base, tamper: { tamp1Open: !!(rs & 0x01), tamp2Open: !!(rs & 0x02) } };
  }
  if (event === 0x13) {
    return { ...base, pressureOk: rs === 0 };
  }
  if (event === 0x01) {
    return { ...base, usageCounter: 0, unmeteredFlowTicks: usageCounter };
  }
  if (event === 0x16) {
...
export const EWC_CMD_NAMES: Record<number, string> = {
  ...
  0x42: "Bulk Clear Supertap Table",
  0x43: "Set Clock",
  0x44: "Datalog Output",
  0x45: "Read EEPROM Byte",
  0x46: "Factory Reset",
  0x4B: "Request to Program",
  0x4C: "Load Supertap UID + Top-Up",
  0x4D: "Version Message",
  0x4E: "Write Tick Accumulator",
  0x4F: "Valve OFF",
  0x50: "Write EEPROM Byte",
  0x52: "Read SPI Log",
  0x53: "Consumer Data Message",
  0x54: "Get Time",
  0x55: "Tap Top-Up",
  0x56: "Valve ON",
  0x58: "Get Status",
  0x5A: "Clear Supertap UID + Top-Up",
  0x65: "Read EEPROM Word",
  0x6D: "Copyright Message",
  0x70: "Write EEPROM Word",
  0x72: "Read Log Pointer",
  0x77: "Write Log Pointer",
};

// ─── 28-byte embedded datalog decoder (inside Read SPI Log replies) ────────────
...
// ─── CommandApi outer JSON decoder ─────────────────────────────────────────────
// Node.js version: uses Buffer.from(..., 'base64') instead of atob().

export type CommandApiArgs =
  | { kind: "credit"; creditMits: number }
  | { kind: "read-log"; logNumber: number }
  | { kind: "set-clock"; timeStr: string }
  | { kind: "eeprom-read"; addr: number }
  | { kind: "eeprom-write"; addr: number; value: number }
  | { kind: "eeprom-word-write"; addr: number; value: number }
  | { kind: "log-pointer-write"; pointer: number };
...
export function decodeCommandApiPayload(base64Json: string): CommandApiDecoded | null {
  try {
    ...
    const innerBytes = Array.from(Buffer.from(innerB64, "base64"));
